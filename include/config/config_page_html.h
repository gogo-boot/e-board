...
<body>
  <div class="container">
    ...
    <div class="chapter">📺 Anzeige-Konfiguration</div>

    <div class="config-item">
      <div class="label">
          Anzeigemodus
          <span class="tooltip">ℹ️
      <span class="tooltiptext">Wählen Sie, was auf dem Display angezeigt werden soll.</span>
    </span>
      </div>
      <div class="config-row">
          <select id="display-mode"
                  ...
                  data-saved-mode="{{DISPLAY_MODE}}">
              <option value="0">Wetter + Abfahrten (Hälfte-Hälfte)</option>
      ...
      <div class="display-mode-preview" id="display-preview">
          <div class="preview-box">
              <div class="preview-half">🌤️</div>
              <div class="preview-half">🚌</div>
          </div>
          <span id="preview-text">Wetter links, Abfahrten rechts</span>
      </div>
      <div class="help-text">
          <strong>Hälfte-Hälfte:</strong> Wetter links, Abfahrten rechts - ideal für beide Informationen.<br>
          <strong>Nur Wetter:</strong> Vollbild-Wetterprognose mit Details und Grafiken.<br>
          <strong>Nur Abfahrten:</strong> Mehr Platz für Verkehrsinformationen und Details.
      </div>
    </div>
    ...
    <div class="config-item">
      <div class="label">
        Mein Standort
        <span class="tooltip">ℹ️
          <span class="tooltiptext">Standort wird für Wetter- und Verkehrsdaten verwendet. Geben Sie Ihre Postleitzahl ein, um die Stadt zu finden.</span>
        </span>
      </div>
      <div class="config-row">
    ...
    <div class="chapter" style="display: flex; align-items: center; justify-content: space-between;">
      ...
      <img alt="Logo: RMV, Rhein-Main-Verkehrsverbund" src="https://www.rmv.de/c/typo3conf/ext/rmvtemplate/Resources/Public/Images/logo.svg" width="148" height="30" style="margin-left: auto;" onerror="this.style.display='none'">
    </div>
    <div class="config-item">
      <div class="label">
        Meine Haltestelle
    ...
    <div class="config-item">
      ...
      <div class="chips" id="filter-chips" style="display: flex; flex-wrap: wrap; gap: 0.3em; justify-content: flex-start;">
        ...
        <div class="chip active" data-type="Ferry" onclick="toggleChip(this)">Fähre</div>
      </div>
    </div>
    ...
    <div class="config-item">
      ...
      <div id="weekend-config" style="display: none; margin-top: 0.5em;">
        <div class="config-row">
          <div class="time-range">
            <span>Wochenende ÖPNV-Abfahrten aktiv:</span>
    ...
    <div class="config-item">
      ...
      <div class="config-row">
        <div id="battery-usage-info" style="background: #e8f5e8; padding: 0.5em; border-radius: 4px; width: 100%;">
          ...
          <div style="margin-top: 0.4em; font-size: 0.9em; font-weight: 500;">
            🔋 Geschätzte Akkulaufzeit: <span id="battery-days-estimate">–</span>
          </div>
        </div>
      </div>
    ...
    <div class="chapter">🔄 Firmware-Aktualisierung</div>
    ...
    <div class="config-item">
      ...
      <div class="help-text">
        Wenn aktiviert, prüft das Gerät täglich auf neue Firmware-Versionen und installiert diese automatisch.
      </div>
    </div>

    <div class="config-item" id="ota-check-time-container">
      <div class="label">
        Prüfzeit für Updates
        <span class="tooltip">ℹ️
          <span class="tooltiptext">Zu welcher Uhrzeit soll täglich nach Updates gesucht werden?</span>
        </span>
      </div>
      <div class="config-row">
      ...
      <div class="help-text">
        Das Gerät prüft täglich zu dieser Uhrzeit, ob eine neue Firmware-Version verfügbar ist.
      </div>
      <div class="warning">
        ⚠️ Während der Aktualisierung ist das Gerät für ca. 1-2 Minuten nicht verfügbar. Die Aktualisierung erfolgt automatisch nach erfolgreicher Prüfung.
      </div>
    </div>
  ...
  <script>
    // Lazy-load location and stops from /api/init after page loads
    (function() {
      var stopSelect = document.getElementById('stop-select');
      var cityDisplay = document.getElementById('city-display');
      var latInput = document.getElementById('city-lat');
      var lonInput = document.getElementById('city-lon');
      var latDisplay = document.getElementById('lat-display');
      var lonDisplay = document.getElementById('lon-display');

      // Show loading state only if no data was pre-filled by server
      if (!latInput.value || latInput.value === '0.000000') {
        cityDisplay.textContent = 'Standort wird ermittelt...';
        stopSelect.innerHTML = '<option value="">Haltestellen werden geladen...</option>';
        ...
        fetch('/api/init')
          ...
          .then(function(data) {
            if (data.lat && data.lon) {
              ...
              latDisplay.textContent = parseFloat(data.lat).toFixed(4);
              lonDisplay.textContent = parseFloat(data.lon).toFixed(4);
            }
            if (data.city) {
          ...
          .catch(function(err) {
            ...
            cityDisplay.textContent = 'Fehler bei Standorterkennung';
            stopSelect.innerHTML = '<option>Fehler beim Laden</option>';
          });
      }
    })();
    ...
    function updateDisplayModePreview() {
      ...
      switch (displayMode) {
          case '0': // Half and half
              previewBox.innerHTML = '<div class="preview-half">🌤️</div><div class="preview-half">🚌</div>';
    ...
    document.getElementById('save-btn').onclick = function() {
      ...
      if ((displayMode === '0' || displayMode === '2') && (!stopId || stopId.trim() === '')) {
          alert('Bitte wählen Sie eine Haltestelle aus, um den Anzeigemodus zu speichern.');
          return;
      }
    ...
    function calculateBatteryUsage() {
      ...
      // --- Akkulaufzeit-Schätzung (basiert auf Tests: 8400 Aktualisierungen pro Ladung) ---
      var TOTAL_REFRESHES_PER_CHARGE = 8400;
      var estimateEl = document.getElementById('battery-days-estimate');
      if (totalCallsPerDay > 0) {
        var estimatedDays = Math.round(TOTAL_REFRESHES_PER_CHARGE / totalCallsPerDay);
        var durationLabel = '';
        if (estimatedDays >= 60) {
          var months = Math.floor(estimatedDays / 30);
          durationLabel = 'ca. ' + estimatedDays + ' Tage (' + months + ' Monate)';
        } else if (estimatedDays >= 14) {
          var weeks = Math.floor(estimatedDays / 7);
          durationLabel = 'ca. ' + estimatedDays + ' Tage (' + weeks + ' Wochen)';
        } else {
          durationLabel = 'ca. ' + estimatedDays + ' Tage';
        }
        // Color coding
        if (estimatedDays >= 90) {
          estimateEl.style.color = '#1a7f37';
        } else if (estimatedDays >= 60) {
          estimateEl.style.color = '#856404';
        } else if (estimatedDays >= 30) {
          estimateEl.style.color = '#c75000';
        } else {
          estimateEl.style.color = '#cc0000';
        }
        estimateEl.textContent = durationLabel;
      } else {
    ...
    setTimeout(function() {
      ...
      document.addEventListener('DOMContentLoaded', function () {
          // Pre-select saved display mode
          var displayModeSelect = document.getElementById('display-mode');
          var savedMode = displayModeSelect.getAttribute('data-saved-mode');
    ...
    // --- iOS tooltip fix (touchstart toggle) ---
