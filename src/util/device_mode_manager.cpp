...
void DeviceModeManager::runConfigurationMode()
{
    ...
    if (g_webConfigPageData.latitude == 0.0 && g_webConfigPageData.longitude == 0.0)
    {
        ...
        ESP_LOGI(TAG, "Fetching city name from lat/lon: (%f, %f)", g_webConfigPageData.latitude,
    ...
    {
        ESP_LOGI(TAG, "Using saved location: %s (%f, %f)", g_webConfigPageData.cityName, g_webConfigPageData.latitude,
                 g_webConfigPageData.longitude);
    }
...
void DeviceModeManager::runOperationalMode()
{
    ...
    ESP_LOGI(TAG, "Using saved location: %s (%f, %f)", config.cityName, g_webConfigPageData.latitude,
             g_webConfigPageData.longitude);
    ...
    if (MyWiFiManager::isConnected())
    {
        ...
        // Fetch weather data
        ESP_LOGI(TAG, "Fetching weather for location: (%f, %f)", g_webConfigPageData.latitude,
                 g_webConfigPageData.longitude);
